...
def analyze_stock(ticker: str) -> ScreenResult | None:   # ✅ ScreenResult ไม่ใช่ screenResult
    try:
        ...
        if ML_MODEL:
            ...
            ml_prob = float(ML_MODEL.predict_proba(feat)[0][1])  # ✅ ไม่มี predict_ ซ้ำ
...
def run_screener(top_n: int = 5) -> list[ScreenResult]:
    ...
    # ✅ ออกจาก loop ก่อนค่อย sort และ return
...
def format_screener_message(results: list[ScreenResult]) -> str:
    ...
    for i, r in enumerate(results):
        arrow = "📈" if r.change_1d > 0 else "📉"  # ✅ แก้ arrow
        msg += (
            f"{medals[i]} **{r.ticker}**\n"
